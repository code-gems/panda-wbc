	...
	private _renderIcon(): TemplateResult | void {
		if (!this.hideIcon) {
			if (String(this.theme).toLocaleLowerCase().includes("spinner")) {
				return html`<panda-spinner .spinner="${this.spinnerType}"></panda-spinner>`;
			} else {
				let icon: string = "";

				if (this.theme.includes("info")) {
					icon = "info";
				}
				if (this.theme.includes("done")) {
					icon = "check-circle";
				}
				if (this.theme.includes("warn")) {
					icon = "warning";
				}
				if (this.theme.includes("alert")) {
					icon = "error";
				}
