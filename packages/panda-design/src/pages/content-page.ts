...
export abstract class ContentPage extends LitElement {
	// page details
	abstract pageId: string;
	abstract contextMenu: ContextMenuItem[];
	...
	private _renderContextMenu() {
		...
		this.contextMenu.forEach(({ name, contextId }) => {
	...
	private _onContextMenuClick(contextId: string) {
		...
		if (contentSectionEl) {
			...
			console.warn("%c Context menu config:", "font-size: 16px;", this.contextMenu);
